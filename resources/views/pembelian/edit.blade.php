...
@section('content')
    <div class="card">
        ...
        <div class="card-body py-4">
            ...
            @endif
            <form id="form-pembelian" action="{{ route('pembelian.update', $pembelian->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="mb-5">
                            ...
                            <input type="text" class="form-control @error('no_po') is-invalid @enderror" name="no_po"
                                value="{{ old('no_po', $pembelian->no_po) }}" placeholder="No PO (opsional)">
                            @error('no_po')
                    ...
                    <div class="col-md-6">
                        <div class="mb-5">
                            ...
                            <input type="text" class="form-control @error('no_faktur') is-invalid @enderror"
                                name="no_faktur" value="{{ old('no_faktur', $pembelian->no_faktur) }}"
                    ...
                    <div class="col-md-6">
                        <div class="mb-5">
                            ...
                            <input type="date" class="form-control @error('tanggal_faktur') is-invalid @enderror"
                                name="tanggal_faktur"
                                value="{{ old('tanggal_faktur', $pembelian->tanggal_faktur->format('Y-m-d')) }}" required>
                    ...
                    <div class="col-md-4">
                        <div class="mb-5">
                            ...
                            <select class="form-select @error('jenis') is-invalid @enderror" name="jenis"
                                ...
                                <option value="TUNAI" {{ old('jenis', $pembelian->jenis) == 'TUNAI' ? 'selected' : '' }}>
                                    TUNAI</option>
                                <option value="HUTANG" {{ old('jenis', $pembelian->jenis) == 'HUTANG' ? 'selected' : '' }}>
                                    HUTANG</option>
                                <option value="KONSINYASI"
                                    {{ old('jenis', $pembelian->jenis) == 'KONSINYASI' ? 'selected' : '' }}>KONSINYASI
                                </option>
                            </select>
                            @error('jenis')
                                <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <select class="form-select @error('akun_kas_id') is-invalid @enderror" name="akun_kas_id"
                                ...
                                @foreach ($akuns as $akun)
                                    <option value="{{ $akun->id }}"
                                        {{ old('akun_kas_id', $pembelian->akun_kas_id) == $akun->id ? 'selected' : '' }}>
                                        {{ $akun->kode }} - {{ $akun->nama }}</option>
                                @endforeach
                            </select>
                            @error('akun_kas_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-4 hutang-section" style="display: none;">
                        <div class="mb-5">
                            ...
                            <input type="date" class="form-control @error('tanggal_jatuh_tempo') is-invalid @enderror"
                                ...
                                value="{{ old('tanggal_jatuh_tempo', $pembelian->tanggal_jatuh_tempo ? $pembelian->tanggal_jatuh_tempo->format('Y-m-d') : '') }}">
                                ...
                                <tr class="detail-row">
                                    <td>
                                        <input type="hidden" name="detail[{{ $index }}][id]"
                                            value="{{ $detail->id }}">
                                        <select class="form-select form-select-sm obat-select"
                                            name="detail[{{ $index }}][obat_id]" required
                                            data-index="{{ $index }}">
                                            <option value="">Pilih Obat</option>
                                            @foreach ($obats as $obat)
                                                <option value="{{ $obat->id }}"
                                                    {{ $detail->obat_id == $obat->id ? 'selected' : '' }}>
                                    ...
                                    <td>
                                        <select class="form-select form-select-sm satuan-select"
                                            name="detail[{{ $index }}][satuan_id]" required>
                                            <option value="">Pilih Satuan</option>
                                            @foreach ($detail->obat->satuans as $satuan)
                                                <option value="{{ $satuan->satuan_id }}"
                                                    {{ $detail->satuan_id == $satuan->satuan_id ? 'selected' : '' }}>
                                                    {{ $satuan->satuan->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td>
                                        <input type="number" class="form-control form-control-sm jumlah-input"
                                            name="detail[{{ $index }}][jumlah]" value="{{ $detail->jumlah }}"
                                            min="1" required>
                                    </td>
                                    <td>
                                        <input type="text"
                                            class="form-control form-control-sm harga-beli-input text-end"
                                            name="detail[{{ $index }}][harga_beli]"
                                            value="{{ number_format($detail->harga_beli, 0, ',', '.') }}" required>
                                    </td>
                                    <td>
                                        <input type="text"
                                            class="form-control form-control-sm subtotal-item-display text-end"
                                            value="{{ number_format($detail->subtotal, 0, ',', '.') }}" disabled>
                                        <input type="hidden" class="subtotal-item-input"
                                            name="detail[{{ $index }}][subtotal]"
                                            value="{{ $detail->subtotal }}">
                                    </td>
                                    <td>
                                        <input type="number" class="form-control form-control-sm diskon-persen-input"
                                            name="detail[{{ $index }}][diskon_persen]"
                                            value="{{ $detail->diskon_persen }}" min="0" max="100"
                                            step="0.01">
                                    </td>
                                    <td>
                                        <input type="text"
                                            class="form-control form-control-sm diskon-nominal-display text-end"
                                            value="{{ number_format($detail->diskon_nominal, 0, ',', '.') }}" disabled>
                                    ...
                                    <td>
                                        <input type="number" class="form-control form-control-sm margin-jual-input"
                                            name="detail[{{ $index }}][margin_jual_persen]"
                                            value="{{ $detail->margin_jual_persen }}" min="0" step="0.01">
                                    </td>
                                    <td>
                                        <input type="text"
                                    ...
                                    <td>
                                        <input type="text" class="form-control form-control-sm"
                                            name="detail[{{ $index }}][no_batch]" value="{{ $detail->no_batch }}"
                                            required>
                                    </td>
                                    <td>
                                        <input type="date" class="form-control form-control-sm"
                                            name="detail[{{ $index }}][tanggal_expired]"
                                            value="{{ $detail->tanggal_expired->format('Y-m-d') }}" required>
                                    </td>
                                    <td>
                                        <select class="form-select form-select-sm"
                                            name="detail[{{ $index }}][lokasi_id]" required>
                                            <option value="">Pilih Lokasi</option>
                                            @foreach ($lokasis as $lokasi)
                                                <option value="{{ $lokasi->id }}"
                                                    {{ $detail->stok->first() && $detail->stok->first()->lokasi_id == $lokasi->id ? 'selected' : '' }}>
                                                    {{ $lokasi->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td>
                                        <input type="text"
                                            class="form-control form-control-sm total-item-display text-end"
                                            value="{{ number_format($detail->total, 0, ',', '.') }}" disabled>
                                        <input type="hidden" class="total-item-input"
                                            name="detail[{{ $index }}][total]" value="{{ $detail->total }}">
                                    </td>
                                    <td>
                                        <button type="button"
                                            class="btn btn-sm btn-icon btn-light-danger delete-row-btn">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-8">
                                <input type="text" class="form-control form-control-sm text-end" id="subtotal-display"
                                    value="{{ number_format($pembelian->subtotal, 0, ',', '.') }}" disabled>
                                <input type="hidden" name="subtotal" id="subtotal-input"
                                    value="{{ $pembelian->subtotal }}">
                            </div>
                        </div>
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-8">
                                <input type="text" class="form-control form-control-sm text-end"
                                    id="diskon-total-display"
                                    value="{{ number_format($pembelian->diskon_total, 0, ',', '.') }}" disabled>
                                <input type="hidden" name="diskon_total" id="diskon-total-input"
                                    value="{{ $pembelian->diskon_total }}">
                            </div>
                        </div>
                        <div class="mb-3 row">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-8">
                                <input type="text" class="form-control form-control-sm text-end fw-bolder"
                                    ...
                                    value="{{ number_format($pembelian->grand_total, 0, ',', '.') }}" disabled>
                                <input type="hidden" name="grand_total" id="grand-total-input"
                                    value="{{ $pembelian->grand_total }}">
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            let detailIndex = {{ count($pembelian->details) }};
            ...
            $('#jenis-pembayaran').on('change', function() {
                ...
                if (jenis === 'TUNAI') {
                    $('.tunai-section').show();
                    $('.hutang-section').hide();
                    $('select[name="akun_kas_id"]').attr('disabled', false).prop('required', true);
                    $('input[name="tanggal_jatuh_tempo"]').attr('disabled', true).prop('required', false);
                } else if (jenis === 'HUTANG') {
                    $('.tunai-section').hide();
                    $('.hutang-section').show();
                    $('select[name="akun_kas_id"]').attr('disabled', true).prop('required', false);
                    $('input[name="tanggal_jatuh_tempo"]').attr('disabled', false).prop('required', true);
                } else {
                    $('.tunai-section').hide();
                    $('.hutang-section').hide();
                    $('select[name="akun_kas_id"]').attr('disabled', true).prop('required', false);
                ...
                // Trigger validation
                $("#form-pembelian").validate().element('select[name="akun_kas_id"]');
                $("#form-pembelian").validate().element('input[name="tanggal_jatuh_tempo"]');
            });
            ...
            function addDetailRow() {
                console.log('addDetailRow function called in edit view');
                // Hide empty row
                $('#empty-row').hide();
                ...
                addDetailValidation(detailIndex);
            ...
            function initValidator() {
                $("#form-pembelian").validate({
                    ...
                    messages: {
                        ...
                        akun_kas_id: {
                            required: "Akun kas tidak boleh kosong untuk pembayaran tunai"
                        },
                        tanggal_jatuh_tempo: {
                            required: "Tanggal jatuh tempo tidak boleh kosong untuk pembayaran hutang",
                            date: "Format tanggal tidak valid",
            ...
            function addDetailValidation(index) {
                // No need to add validation rules manually with jQuery validate
                // It automatically picks up the validation rules from HTML attributes
        ...
        function calculateTotals() {
            ...
            $('.detail-row').each(function() {
                ...
                const rowDiskon = parseInt($(this).find('.diskon-nominal-input').val().replace(/[^\d]/g, '')) || 0;
