...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('akun.update', $akun->id) }}" method="POST" id="akunForm">
                @csrf
                @method('PUT')
                <div class="row mb-5">
                    <div class="col-md-6">
                        <div class="mb-5">
                            ...
                            <span class="form-text text-muted">Kode akan otomatis diawali dengan 'AKN'</span>
                            @error('kode')
                        ...
                        <div class="mb-5">
                            ...
                            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                <option value="1" {{ old('status', $akun->status) == '1' ? 'selected' : '' }}>
                                    Aktif</option>
                                <option value="0" {{ old('status', $akun->status) == '0' ? 'selected' : '' }}>Non
                                    Aktif</option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback">{{ $message }}</span>
